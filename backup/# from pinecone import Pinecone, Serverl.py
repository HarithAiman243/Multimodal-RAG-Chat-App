...
class VectorDB:
    ...
    def __init__(self):
        ...
        # Get index name from config file
        index_name = config['pinecone_index_name']
        pc = Pinecone(api_key=os.environ.get('PINECONE_API_KEY'))

        if index_name not in pc.list_indexes().names():
            print(f"Warning: Index '{index_name}' not found. Creating a new empty one.")
            pc.create_index(
                index_name,
                dimension=1536,
                metric='cosine',
        ...
        if db_name == 'pinecone':
            # Note: Open AI embedding dim is 1536
            self.vectorstore = setup_pinecone(index_name, embedding, 1536, 'cosine')
        #else:
